    ...
    def __init__(self, in_planes, planes, stride=1, option='A', **kwargs):
        ...
        if stride != 1 or in_planes != planes:
            if option == 'A':
                ...
                #self.shortcut = LambdaLayer(lambda x:F.pad(x[:, :, ::2, ::2], (0, 0, 0, 0, planes, planes), "constant", 0))
            elif option == 'B':
                self.shortcut = nn.Sequential(
...
def resnet92(num_classes=10, **kwargs):
    return ResNet(BasicBlock, [15, 15, 15], num_classes=num_classes, **kwargs)


def resnet110(num_classes=10, **kwargs):
    return ResNet(BasicBlock, [18, 18, 18], num_classes=num_classes, **kwargs)

class multi_exit_ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, **kwargs):
        super(multi_exit_ResNet, self).__init__()
        self.in_planes = kwargs['channel']
        ...
        self.layer2 = self._make_layer(block, kwargs['channel']*2, num_blocks[1], stride=2, **kwargs)
        self.layer3 = self._make_layer(block, kwargs['channel']*4, num_blocks[2], stride=2, **kwargs)
        self.linear1 = nn.Linear(kwargs['channel'], num_classes, bias=False)
        self.linear2 = nn.Linear(kwargs['channel']*2, num_classes, bias=False)
        self.linear3 = nn.Linear(kwargs['channel']*4, num_classes, bias=False)
        self.relu = nn.ReLU() 
        self.ii=0
        #self.relu = q.PactReLU() if kwargs['pact'] else nn.ReLU()
...
def resnet56_multi_exit(num_classes=10, **kwargs):
    return multi_exit_ResNet(BasicBlock, [9, 9, 9], num_classes=num_classes, **kwargs)
def resnet44_multi_exit(num_classes=10, **kwargs):
    return multi_exit_ResNet(BasicBlock, [7, 7, 7], num_classes=num_classes, **kwargs)
def resnet110_multi_exit(num_classes=10, **kwargs):
    return multi_exit_ResNet(BasicBlock, [18, 18, 18], num_classes=num_classes, **kwargs)
